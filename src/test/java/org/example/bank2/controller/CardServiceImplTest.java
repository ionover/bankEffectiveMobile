...
public class CardServiceImplTest {
    ...
    void setUp() {
        cardService = new CardServiceImpl(repository,
                                          userService,
                                          new CardNumberProtector(
                                                  "testCardEncryptionSecret",
                                                  "testCardHashSecret"));
    }
    ...
    void getAllCardsWithoutFiltersReturnsAllCardsWithPaginationForAdmin() {
        ...
        when(repository.findAllByFilters(null, null, null, null, pageable)).thenReturn(cardsPage);
        ...
        verify(repository).findAllByFilters(null, null, null, null, pageable);
    }
    ...
    void getAllCardsWithNumberFilterReturnsDifferentCardsForAdmin() {
        ...
        when(repository.findAllByFilters(null, numberFilter, null, null, pageable)).thenReturn(cardsPage);
        ...
        assertEquals("**** **** **** 0001", result.getContent().getFirst().getNumber());
        verify(repository).findAllByFilters(null, numberFilter, null, null, pageable);
    }
    ...
    void getAllCardsWithStatusFilterReturnsDifferentCardsForAdmin() {
        ...
        when(repository.findAllByFilters(null, null, CardStatus.FRIEZE, null, pageable)).thenReturn(cardsPage);

        Page<CardResponse> result = cardService.getAllCards(pageable, null, CardStatus.FRIEZE, null);

        assertEquals(1, result.getContent().size());
        assertEquals(CardStatus.FRIEZE, result.getContent().getFirst().getStatus());
        assertEquals(2, result.getTotalElements());
        verify(repository).findAllByFilters(null, null, CardStatus.FRIEZE, null, pageable);
    }
    ...
    void getAllCardsWithBalanceFilterReturnsDifferentCardsForAdmin() {
        ...
        when(repository.findAllByFilters(null, null, null, balanceFilter, pageable)).thenReturn(cardsPage);

        Page<CardResponse> result = cardService.getAllCards(pageable, null, null, balanceFilter);

        assertEquals(1, result.getContent().size());
        assertEquals(15_000L, result.getContent().getFirst().getBalance());
        assertEquals("**** **** **** 0005", result.getContent().getFirst().getNumber());
        verify(repository).findAllByFilters(null, null, null, balanceFilter, pageable);
    }
    ...
    void getAllCardsWithAllFiltersReturnsDifferentCardsForCurrentUser() {
        ...
        when(repository.findAllByOwnerIdAndFilters(77L, null, numberFilter, statusFilter, balanceFilter, pageable))
                .thenReturn(cardsPage);

        Page<CardResponse> result = cardService.getAllCards(pageable, numberFilter, statusFilter, balanceFilter);

        assertEquals(1, result.getContent().size());
        assertEquals(77L, result.getContent().getFirst().getOwnerId());
        assertEquals("**** **** **** 1234", result.getContent().getFirst().getNumber());
        verify(userService).getUserProjectionByLogin("user-login");
        verify(repository).findAllByOwnerIdAndFilters(77L, null, numberFilter, statusFilter, balanceFilter, pageable);
    }
    ...
    private Card card(Long id, String number, Long ownerId, CardStatus status, Long balance) {
        Card card = new Card();
        card.setId(id);
        card.setNumberEncrypted("encrypted-" + id);
