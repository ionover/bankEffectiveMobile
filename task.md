...
Проектная структура с директориями и описательными файлами (README Controller.md, README Service.md и т.д.) уже подготовлена (https://gitlab.com/PaatoM/bank\_rest). Все реализации нужно добавлять в соответствующие директории. После завершения разработки временные README-файлы нужно удалить, чтобы они не попадали в итоговую сборку.
...
Весь код и изменения должны быть выложены в **отдельный git-репозиторий исполнителя** (GitHub публичный хостинг с открытым доступом к проекту). Предоставление проекта в виде файлов или архивов **не допускается**.